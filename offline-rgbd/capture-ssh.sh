...
remote_command() {
    ...
    echo "cmd /k \\\"C:/Program Files/Azure Kinect SDK v1.4.1/tools/k4arecorder\\\" --device ${devicenum} --color-mode ${colormode} --depth-mode ${depthmode} --rate ${rate} --imu OFF --external-sync ${syncmode} --sync-delay ${syncdelay} --exposure-control 20000 ${hostname}_${devicenum}_${sessionid}.mkv 2^> ${hostname}_${devicenum}_${sessionid}_error.txt"
}
...
sleep 1
remote_cmd=`remote_command disnuc301 ${sessionname} 0 Sub 480`
